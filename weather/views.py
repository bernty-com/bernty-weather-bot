...
from .models import City, Country, Forecast, FavoriteCity
...
# не решено. Надо взять настоящего пользователя
        user =  User.objects.all() #auth.get_user(request)
        context['fc'] = Forecast(pk)
        context['is_favorited'] = FavoriteCity.objects.filter(city__exact=pk, user__exact=user).exists()
        return context
